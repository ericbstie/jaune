...
test("rejects failed or interrupted streams", async () => {
  const deltas: string[] = [];
  await expect(readReply(stream('{"delta":"partial"}\n'), (delta) => deltas.push(delta))).rejects.toThrow("before completion");
  await expect(readReply(stream('{"error":"Failed"}\n'), (delta) => deltas.push(delta))).rejects.toThrow("Reply failed");
});
