...
async function request(fixture: TestServer, path: string, body?: unknown, token = fixture.session.token): Promise<Response> {
...
test("streams before completion, persists roles and rejects duplicate and foreign replies", async () => {
...
test("keeps user messages but never saves partial replies", async () => {
